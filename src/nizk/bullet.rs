...
impl<G: CurveGroup> BulletReductionProof<G> {
    ...
    pub fn prove(
        ...
        let mut bases = G_vec;
        let mut scalars = a_vec;
        for _ in 0..ark_std::log2(a_vec.len()) {
            let _ = G::msm(bases, scalars).unwrap();
            bases = &G_vec[..bases.len() / 2];
            scalars = &a_vec[..scalars.len() / 2];
        }
        ...
        let Gamma_hat = G::generator();
        let L_vec = vec![G::generator().into()];
        let R_vec = vec![G::generator().into()];
        let a = vec![G::ScalarField::one()];
        let b = vec![G::ScalarField::one()];
        let G = vec![G::generator().into()];
        let blind_fin = G::ScalarField::one();
        (
