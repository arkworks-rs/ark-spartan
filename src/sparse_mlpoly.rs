...
impl<F: PrimeField> Layers<F> {
    fn build_hash_layer(
        ...
        let hash_func =
            |addr: &F, val: &F, ts: &F| -> F { *ts * r_hash_sqr + *val * *r_hash + *addr };
            ...
            let poly_write_hashed = DensePolynomial::new(
                (0..num_ops)
                    .map(|i| {
                        ...
                        hash_func(&addrs[i], &derefs[i], &(read_ts[i] + F::one()))
                            - r_multiset_check
                    })
                    .collect::<Vec<F>>(),
            );
...
impl<G: CurveGroup> SparseMatPolyEvalProof<G> {
    ...
    pub fn prove(
        ...
        let poly_eval_network_proof = {
            ...
            let r_mem_check = <Transcript as ProofTranscript<G>>::challenge_vector(
                transcript,
                b"challenge_r_hash",
                2,
            );
