...
impl<G: CurveGroup> R1CSProof<G> {
    ...
    pub fn prove(
        ...
        let r_A =
            <Transcript as ProofTranscript<G>>::challenge_scalar(transcript, b"challenege_Az");
        let r_B =
            <Transcript as ProofTranscript<G>>::challenge_scalar(transcript, b"challenege_Bz");
        let r_C =
            <Transcript as ProofTranscript<G>>::challenge_scalar(transcript, b"challenege_Cz");
    ...
    pub fn verify(
        ...
        let (num_rounds_x, num_rounds_y) =
            (num_cons.log_2() as usize, (2 * num_vars).log_2() as usize);
        ...
        let claim_phase1 =
            G::ScalarField::zero().commit(&G::ScalarField::zero(), &gens.gens_sc.gens_1);
        ...
        let taus_bound_rx: G::ScalarField = (0..rx.len())
            .map(|i| {
                rx[i] * tau[i] + (G::ScalarField::one() - rx[i]) * (G::ScalarField::one() - tau[i])
            })
            .product();
        ...
        let comm_claim_phase2 =
            VariableBaseMSM::msm(bases_affine.as_ref(), scalars.as_ref()).unwrap();
        ...
        let poly_input_eval = {
            // constant term
            let mut input_as_sparse_poly_entries =
                vec![SparsePolyEntry::new(0, G::ScalarField::one())];
            //remaining inputs
